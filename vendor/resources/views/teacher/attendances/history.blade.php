...
<style>
    ...
    .status-Hadir { background-color: #d1fae5; color: #065f46; } /* green-100, green-800 */
    .status-Sakit { background-color: #dbeafe; color: #1e40af; } /* blue-100, blue-800 */
    .status-Izin { background-color: #fef3c7; color: #92400e; } /* yellow-100, yellow-800 */
    .status-Alpha { background-color: #fee2e2; color: #991b1b; } /* red-100, red-800 */
    ...
    .animate-fade-in, .animate-slide-up-item {
        opacity: 0;
    }
    ...
    <div class="attendance-card bg-white mb-6 animate-fade-in">
        <div class="gradient-header px-6 py-4 flex justify-between items-center">
            ...
            <a href="{{ route('teacher.attendances.create') }}" 
                ...
                <i class="fas fa-plus mr-2"></i> Catat Absensi Baru
            </a>
        </div>
        <div class="p-6">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse ($attendances as $attendance)
                        <tr class="hover:bg-gray-50 transition duration-150 ease-in-out animate-slide-up-item">
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{{ $loop->iteration + ($attendances->currentPage() - 1) * $attendances->perPage() }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $attendance->date }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $attendance->student->name ?? 'N/A' }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $attendance->student->class->name ?? 'N/A' }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $attendance->schedule->subject->name ?? 'N/A' }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ \Carbon\Carbon::parse($attendance->schedule->start_time)->format('H:i') }} - {{ \Carbon\Carbon::parse($attendance->schedule->end_time)->format('H:i') }}</td>
                    ...
                    @empty
                        <tr>
                            <td colspan="8" class="px-6 py-4 whitespace-nowrap text-center text-sm text-gray-500">Tidak ada data riwayat absensi.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
