...
<div class="mx-auto py-2">
    <div id="page-title" class="flex items-center mb-8 border-b-2 border-indigo-500 animate-fade-in" style="animation-delay: 0.1s">
    ...
    <div id="teaching" class="tab-content active">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @foreach ($schedulesToday as $schedule)
                                <tr class="hover:bg-gray-50 transition duration-150 ease-in-out">
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $schedule->at_time ?? '-' }}</td>
        ...
        <div class="dashboard-card bg-white mt-6">
            ...
            <div class="p-0 overflow-x-auto rounded-b-xl"> {{-- Added overflow-x-auto and rounded-b-xl for consistency --}}
                @if ($latestJournals->isEmpty())
                    <p class="p-6 text-gray-500">Belum ada jurnal harian terbaru.</p>
                @else
                    <table class="min-w-full divide-y divide-gray-200"> {{-- Converted to table --}}
                        <thead class="bg-gray-50">
                            <tr>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @foreach ($latestJournals as $journal)
                                <tr class="hover:bg-gray-50 transition duration-150 ease-in-out">
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ \Carbon\Carbon::parse($journal->date)->format('d M Y') }}</td>
                                    {{-- Pastikan journal->schedule tersedia dari eager loading di controller --}}
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $journal->schedule->at_time ?? '-' }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                        {{ ($journal->schedule && $journal->schedule->start_time && $journal->schedule->end_time) ? 
                                            \Carbon\Carbon::parse($journal->schedule->start_time)->format('H:i') . ' - ' . \Carbon\Carbon::parse($journal->schedule->end_time)->format('H:i') : 
                                            'N/A' }}
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $journal->class->name ?? 'N/A' }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $journal->subject->name ?? 'N/A' }}</td>
                                    <td class="px-6 py-4 whitespace-normal text-sm text-gray-700">{{ Str::limit($journal->title, 70) ?? 'Tidak ada' }}</td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
                ...
                <div class="p-0">
                    <ul class="divide-y divide-gray-200">
                        ...
                        <li class="py-3 px-6 text-gray-700 flex justify-between items-center hover:bg-gray-50 transition duration-150 ease-in-out">
                            ...
                            <span class="inline-flex items-center justify-normal px-2.5 py-0.5 rounded-full text-xs font-medium bg-indigo-100 text-indigo-800">
                    ...
                    <ul class="divide-y divide-gray-200">
                        ...
                        @forelse ($latestAssessments as $assessment)
                            <li class="py-3 px-6 text-gray-700 hover:bg-gray-50 transition duration-150 ease-in-out">
                                ...
                                {{ $assessment->subject->name ?? 'N/A' }}: <strong class="text-indigo-600">{{ $assessment->score }}</strong>
                            </li>
                        @empty
