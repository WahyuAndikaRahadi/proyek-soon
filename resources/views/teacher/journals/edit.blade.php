...
<script>
    ...
    function confirmEdit() {
        Swal.fire({
            ...
            text: "Anda akan mengubah data jurnal ini!",
            icon: 'question',
...
<div class="mx-auto py-2">
    ...
    <div class="journal-card bg-white">
        ...
        <form method="POST" action="{{ route('teacher.journals.update', parameters: $journal->id) }}" id="editForm">
            @csrf
            @method('PUT')
            <div class="p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                    ...
                    <div class="form-group">
                     ...
                     <div class="form-group">
                        <label for="at_time" class="block text-gray-700 text-sm font-medium mb-2">
                            <i class="fas fa-sort-numeric-up mr-2 text-yellow-500"></i> Jam Ke-
                        </label>
                        <input type="text" name="at_time" id="at_time"
                            class="input-focus w-full p-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-yellow-200 transition @error('at_time') border-red-500 @enderror"
                            value="{{ $journal->at_time ? 'Jam ke-' . $journal->at_time : '-' }}" readonly> {{-- Mengakses at_time langsung dari objek journal --}}
                        @error('at_time')
                        <p class="text-red-500 text-xs italic mt-2">
                            <i class="fas fa-exclamation-circle mr-1"></i> {{ $message }}
                        </p>
            ...
            <div class="px-6 py-4 border-t border-gray-200 bg-gray-50 flex flex-col sm:flex-row justify-end space-y-3 sm:space-y-0 sm:space-x-3">
                <button type="button" onclick="confirmEdit()"
                    class="bg-yellow-600 hover:bg-yellow-700 text-white font-bold py-2 px-4 rounded-lg shadow-md transition duration-200 ease-in-out transform hover:scale-105">
