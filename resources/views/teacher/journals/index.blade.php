...
@section('content')
    ...
    <script>
        ...
        function confirmDelete(event) {
            ...
            // Menargetkan form yang mentrigger event ini
            ...
            Swal.fire({
                ...
                // HANYA jika pengguna menekan "Ya, Hapus!"
                if (result.isConfirmed) {
                    // Submit form yang sudah kita targetkan sebelumnya
                    form.submit();
                }
            });
        }
    ...
    <div class="mx-auto py-2">
        <div id="page-title" class="flex items-center mb-8 border-b-2 border-indigo-500">
            ...
            <div>
                <h1 class="text-2xl md:text-3xl font-bold text-gray-800">Jurnal Harian Saya</h1>
                <p class="text-gray-600">Daftar lengkap jurnal kegiatan pembelajaran</p>
            </div>
        </div>

        <!-- Filter Jurnal -->
        <div class="journal-card bg-white mb-6 filter-card">
            <div class="gradient-header px-6 py-4 flex justify-between items-center">
                <h3 class="text-xl font-semibold text-white">Filter Jurnal</h3>
            ...
            <div class="p-6">
                <form action="{{ route('teacher.journals.index') }}" method="GET">
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                        <div class="form-group">
                            ...
                            <input type="date" name="date" id="date"
                                class="input-focus w-full p-2.5 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-200 transition"
                                value="{{ $request->date }}">
                        </div>
        ...
        <!-- List Jurnal -->
        <div class="journal-card bg-white list-card">
            <div class="gradient-header px-6 py-4">
                <h3 class="text-xl font-semibold text-white">Daftar Jurnal Harian</h3>
            </div>
                ...
                <table class="min-w-full divide-y divide-gray-200 responsive-table">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @forelse ($journals as $journal)
                            <tr class="journal-row transition">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                    {{ $journal->class->name ?? 'N/A' }}
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                    {{ $journal->subject->name ?? 'N/A' }}
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                    {{-- Mengakses at_time langsung dari objek journal --}}
                                    ( {{ $journal->at_time ? 'Jam ke-' . $journal->at_time : '-' }} )
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                    {{ \Carbon\Carbon::parse($journal->start_time)->format('H:i') }} -
                                    {{ \Carbon\Carbon::parse($journal->end_time)->format('H:i') }}
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium action-btns">
                                    <div class="flex space-x-2">
                                        <a href="{{ route('teacher.journals.edit', $journal->id) }}"
                                            class="action-btn flex items-center justify-center px-3 py-1.5 bg-indigo-500 hover:bg-indigo-600 text-white rounded-md shadow-sm">
                                            <i class="fas fa-edit mr-1"></i> Edit
                                        </a>
                                        <form action="{{ route('teacher.journals.destroy', $journal->id) }}"
            ...
            @if ($journals->hasPages())
                <div class="px-6 py-4 border-t border-gray-200 bg-gray-50">
                    {{ $journals->appends($request->except('page'))->links('vendor.pagination.tailwind') }}
                </div>
            @endif
        </div>
    </div>
